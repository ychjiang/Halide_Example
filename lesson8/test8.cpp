...
int main(int argc, char **argv) {
    ...
    // 多次函数来改变矩阵内部的数据计算方式
    {
        ...
        //接下来的定义可以在等式两边计算表达式，最简单的方式如下：
        f(3, 7) = 42;//矩阵指定位置处的值是42
        ...
        //首先定义一个纯净的方案 x+y
        //当计算完之后，将 g(2,1)值改变成42
        //接下来没一行的第一个值等于每一行的第二个值。
        //就是可以改变矩阵内部的任意一个点的值
        g.trace_loads();
        g.trace_stores();
    ...
    // 在循环的内部进行更新
    {
        ...
        // 我们想对前50行数据进行平方操作，按照上面的方案，我们可以写
        // 50个独立的定义一次的执行
        ...
        // 但是，如果我们将上述操作放到循环内部来做，将会方便管理和便于操作
        // 我们通过定义一个“减少区域”并在一个更新环节中使用它来实现效果。
        RDom r(0, 50);//定义一个区域,0-50
        f(x, r) = f(x, r) * f(x, r);//定义一个函数的操作方案
        Buffer<float> halide_result = f.realize(100, 100);//内部实现
    ...
    // 接下来将研究这种更新定义的实际用途：计算一个直方图
    {
        ...
        // 一些图像的操作无法清楚的用一个纯净的函数来表示输出坐标到存储位置
        // 比较经典的例子是计算一个直方图，常规的做法是通过迭代计算输入数据
        // 更新直方图的桶柱。
        // 下面我们将利用Halide来实现上述操作
        ...
        RDom r(0, input.width(), 0, input.height());//定义一个变化区域，图像的所有区域
        ...
        histogram(input(r.x, r.y)) += 1;//每个点的值加1
        ...
        for (int r_y = 0; r_y < input.height(); r_y++) {
            for (int r_x = 0; r_x < input.width(); r_x++) {
                c_result[input(r_x, r_y)] += 1;//通过坐标点找到指定的像素值
                 //像素值对应的result出的数值加上1
            }
        }
    ...
    // Scheduling update steps
    {
        ...
        // 为了控制这些变量，我们使用之前的方案。
        // 向量化并行化
        f.vectorize(x, 4).parallel(y);//沿着y方向的并行化操作
        ...
        // use y.
        // 我们都过update函数来获得函数操作的句柄，这样可以调度这个方案
        // 这个操作是第一次在x上进行向量化操作
        // 在这个更新操作中，我们不能对y进行任何操作，因为并没有使用 y
        f.update(0).vectorize(x, 4);//更新

        // Now we parallelize the second update step in chunks of size
        //现在并行化直方图桶柱的大小    
        Var yo, yi;
        f.update(1).split(y, yo, yi, 4).parallel(yo);//分割操作
    ...
    //上面的方案描述了在函数内部的变量管理策略，但是对于一个producer-consumer模式
    //下是如何计算和存储的呢？
    {
        ...
        // trivial example:
        // 因为更新会对一个存储的数组进行多次操作，因此内联是没有意义的
        // 因此默认的策略尽可能的做最相近的操作
        // 它在循环的内部计算consumer
